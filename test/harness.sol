...
contract harness {
    ...
    // modular : we note that the solmateSqrt and MathMasters share the same pice of code in the bottom , so the different is only
    // with the top , we assume that if the bottom is same so the top should generate the same result from both codes , 


    function solmatTopHalf (uint256 x) external pure returns(uint256 z)  {
     assembly {
            let y := x
